...
import UIKit
    ...
    class InnerGreen: UIView {
        ...
        override func drawRect(rect: CGRect)
        {
            ...
            let greenCenter = CGPointMake(self.frame.size.width/2,self.frame.size.height/2 )
            
            
            let smallRadius = CGFloat(CGFloat(self.frame.size.width)/10)
            CGContextSetStrokeColorWithColor(innerContext, UIColor.greenColor().CGColor)
            ...
            CGContextAddArc(innerContext, greenCenter.x, greenCenter.y, CGFloat(smallRadius), CGFloat(startAngle), CGFloat(endAngle), 1)
