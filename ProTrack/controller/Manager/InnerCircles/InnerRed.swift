...
class InnerRed: UIView {
    ...
    override func drawRect(rect: CGRect)
    {
        ...
        let redCenter = CGPointMake(self.frame.size.width/2,self.frame.size.height/2 )
        
        
        let smallRadius = CGFloat(CGFloat(self.frame.size.width)/10)
        CGContextSetStrokeColorWithColor(innerContext, UIColor.redColor().CGColor)
        ...
        CGContextAddArc(innerContext, redCenter.x, redCenter.y, CGFloat(smallRadius), CGFloat(startAngle), CGFloat(endAngle), 1)
